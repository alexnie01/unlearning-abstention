...
| 1 | B4 install-then-test (**next**) | On the base model under +2×gap at layer 8 — which 07 shows abstains on 66% of questions with no degeneracy — rerun the 06 discrimination probe. If recognition survives while generation abstains, steering reproduces IdkNLL's profile exactly and "learned abstention" gains a synthetic reference implementation to compare methods against. If recognition collapses too, then even an artificially installed gate is not a clean gate, and the knows-but-abstains frame is narrower than it looks. Cheap: one model, one hook, the existing probe. | — |
| 1.5 | B6 can NPO be made to talk? (**new, highest value**) | NPO keeps base-level recognition (0.68) but produces the fact 3% of the time against base's 39%. Search for ANY intervention at layers 8/12 that raises NPO's recall: translation along −epistemic, along the base-minus-NPO differential shift from 03, and projection replacement (B0's operator set), swept over magnitude and scored by 09's recall judge rather than log-probs. | If nothing raises NPO's recall above what the same perturbation does to the base model, then "recognition retained, production suppressed" is not a removable gate either — the strongest version of this project's negative. |
